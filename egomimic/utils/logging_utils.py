...
def _local_wandb_run_id(output_dir: Path) -> str | None:
    ...
    if not stem.startswith("run-") or not stem.endswith(".wandb"):
        return None
    timestamp_and_id = stem[len("run-") : -len(".wandb")]
    return timestamp_and_id.split("-", 1)[1] if "-" in timestamp_and_id else None
...
def persist_wandb_run_identity(cfg: DictConfig, loggers, trainer) -> None:
    ...
    if not loggers or not getattr(trainer, "is_global_zero", True):
        return
    wandb_logger = next(
