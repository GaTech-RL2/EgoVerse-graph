...
| 260M figures + raw data + plot script | `results/figures/`, `results/*.csv`, `tools/plot_codec_rollout_results.py` (in the worktree) |
| Bar charts from the earlier sweep | `/Users/rpunamiya/Desktop/GEAR/arc_eval_out/arc_{variants_overview,axis_sweeps,outcome_breakdown}.png` (**PNGs only — see §3e**) |
...
**Figures:** `results/figures/codec_260m_overview.png` (metrics + CIs, sorted per-episode
curves, paired deltas, outcome breakdown) and `results/figures/codec_260m_vs_40m.png`
(scale comparison). Regenerate with `python tools/plot_codec_rollout_results.py`. Raw
per-episode arrays are committed at `results/usevalpaper4_ep_coverages.csv`.
...
### 3e. What is NOT reproducible — read before trusting §3c

The per-episode `ep_coverages` arrays for §3a and §3b are **committed** at
`results/usevalpaper4_ep_coverages.csv` and `results/usevaldurM_ep_coverages.csv`
(also still retrievable from workflows `usevalpaper4-1` / `usevaldurM-1`).
**The arrays behind §3c are not.**

- The three bar charts in `arc_eval_out/` were generated by a script that was written
  inline and never saved to disk. Only the PNGs survive. There is no input data file.
- The workflows those numbers came from have aged out of `osmo workflow list`, and probing
  their likely names returns `Workflow … is not found`.

So §3c is a **summary-statistics-only** record. You can quote the means and SRs, but you
**cannot** run paired tests against them, recompute a different threshold, or extend the
charts with the 260M runs. If any of that matters, those cells must be re-evaluated.

Concretely: preserve the `ep_coverages` array out of every future `SUMMARY` line into a
committed file at the time you read it. Both times this was skipped, the data was gone
