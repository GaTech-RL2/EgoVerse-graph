...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if memory_kv_cache is not None:
            ...
            k, v = map(
                lambda t: t.view(B_mem, T_mem, self.n_head, self.head_dim).transpose(
                    1, 2
                ),
                (k, v),
            )
...
class AutoregressiveModel(ModuleAttrMixin):
    ...
    def generate(
        ...
        for block in self.blocks:
            ...
            k_mem, v_mem = map(
                lambda t: t.view(
                    B_mem, T_mem, self.n_head, self.n_emb // self.n_head
                ).transpose(1, 2),
                (k_mem, v_mem),
            )
        ...
        for i, block in enumerate(self.blocks):
            x, present = block(
                x, memory, layer_past=None, memory_kv_cache=memory_kv_cache[i]
            )
        ...
        out_tokens = prefix
        finished = (
            torch.zeros(B_mem, dtype=torch.bool, device=prefix.device)
        ...
        for i in range(max_new_tokens):
            ...
            if temperature > 0:
                ...
                next_token = torch.argmax(
                    logits.squeeze(1), dim=-1, keepdim=True
                )  # [B, 1]
