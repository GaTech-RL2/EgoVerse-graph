...
def _checkpoint_model_config(ckpt: dict, config_path: str | None):
    ...
    embedded_model = OmegaConf.select(embedded_cfg, "model")
    if embedded_model is None:
        raise RuntimeError("checkpoint config_tree has no model subtree")
    has_legacy_algo = OmegaConf.select(embedded_cfg, "model.robomimic_model") is not None
    has_pipeline = OmegaConf.select(embedded_cfg, "model.pipeline") is not None
    if has_legacy_algo == has_pipeline:
        raise RuntimeError(
            "checkpoint model must contain exactly one of robomimic_model or pipeline"
        )
    ...
    if config_path is not None:
        ...
        embedded_model = OmegaConf.to_container(
            embedded_cfg.model, resolve=True
        )
        resolved_model = OmegaConf.to_container(
            full_cfg.model, resolve=True
        )
        if embedded_model != resolved_model:
            raise RuntimeError(
                "resolved config model subtree differs from the checkpoint-embedded "
                "model config"
            )
    return embedded_cfg, hparams, full_cfg
...
def _resolve_action_chunk_execution_slice(
    ...
    if horizon <= 0 or start < 0 or start >= horizon:
        raise ValueError(
            f"invalid action execution slice start={start} horizon={horizon}"
        )
    count = horizon - start if replan_every is None else int(replan_every)
    if count <= 0 or start + count > horizon:
        raise ValueError(
            f"invalid replan_every={count} for start={start} horizon={horizon}"
        )
    return start, start + count


class _ActionFlowRolloutAlgo:
    """Evaluator-only deployment bridge for the checkpoint-pinned Action Flow graph.

    It owns normalization, native U-Socket decoding, and action queueing only. The
    learned graph and state tree remain the exact checkpoint-instantiated PipelineAlgo.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.domains = [str(domain)]
        emb_id = int(get_embodiment_id(domain))
        self.domain_by_id = {emb_id: str(domain)}
        self.action_horizon = int(action_horizon)
        self.model_token_dim = int(action_dim)
        if self.action_horizon <= 0 or self.model_token_dim != 4:
            raise RuntimeError(
                "Action Flow rollout currently requires positive H and U-Socket rotvec4"
            )
        action_keys = [
            key
        ...
        if action_keys != ["actions"]:
            raise RuntimeError(
                f"Action Flow rollout expected one 'actions' key, got {action_keys}"
            )
        self.action_key = action_keys[0]
        self._native_decoder = USocketRotVecNativeDecoder()
        self.action_chunk_start_index = 0

        obs_steps = {
            int(stage.n_obs_steps)
            for stage in self.policy.stages
            if hasattr(stage, "n_obs_steps")
        }
        if obs_steps != {1}:
            raise RuntimeError(
                f"Action Flow rollout requires exactly observation horizon 1, got {obs_steps}"
            )

    @property
    def nets(self):
        return self.algo.nets

    @nets.setter
    def nets(self, value):
        self.algo.nets = value
    ...
    def device(self, value):
        self.algo.device = value

    @property
    def policy(self):
        return self.algo.pipeline

    def rollout_adapter_for(self, domain):
        if isinstance(domain, int):
            if domain not in self.domain_by_id:
                raise KeyError(f"unknown Action Flow embodiment id {domain}")
        elif str(domain) not in self.domains:
            raise KeyError(f"unknown Action Flow domain {domain!r}")
        return self._native_decoder

    def process_batch_for_training(self, batch: Mapping) -> OrderedDict:
        """Expose validation batches under numeric embodiment keys for SimEval."""
        from egomimic.rldb.embodiment.embodiment import get_embodiment_id

        processed = OrderedDict()
        for domain, value in batch.items():
            if str(domain) not in self.domains:
                raise KeyError(f"unexpected Action Flow validation domain {domain!r}")
            processed[int(get_embodiment_id(domain))] = _move_tensors(
                dict(value), self.device
            )
        return processed

    def _prepare_rollout_observation(self, obs_zarr: Mapping, emb_id: int) -> dict:
        raw = dict(obs_zarr)
        for key in self.norm_stats.keys_of_type("action_keys", emb_id):
            raw.pop(key, None)
            zarr_key = self.norm_stats.keyname_to_zarr_key(key, emb_id)
            if zarr_key is not None:
                raw.pop(zarr_key, None)
        normalized = self.norm_stats.normalize(raw, emb_id)
        mapped = {}
        for key, value in normalized.items():
            model_key = self.norm_stats.zarr_key_to_keyname(key, emb_id)
            if model_key is not None:
                mapped[model_key] = value
        required = {"front_img_1", "state_agent_obj"}
        if set(mapped) != required:
            raise RuntimeError(
                f"Action Flow rollout observations differ: expected {sorted(required)}, "
                f"got {sorted(mapped)}"
            )
        return _move_tensors(mapped, self.device)

    @torch.inference_mode()
    def _predict_native_chunk(self, obs_zarr: Mapping, emb_id: int):
        domain = self.domain_by_id[emb_id]
        model_batch = self._prepare_rollout_observation(obs_zarr, emb_id)
        result = self.algo.forward_eval(OrderedDict(((domain, model_batch),)))[domain]
        normalized = result.get("pred_action")
        if not torch.is_tensor(normalized):
            raise RuntimeError("Action Flow inference did not emit tensor pred_action")
        tokens = self.norm_stats.unnormalize(
            {self.action_key: normalized}, emb_id
        )[self.action_key]
        decoded = self._native_decoder.decode(tokens, context={})
        if not torch.is_tensor(decoded):
            decoded = torch.as_tensor(decoded)
        if tuple(decoded.shape) != (1, self.action_horizon, 3):
            raise RuntimeError(
                "Action Flow native chunk shape mismatch: "
                f"{tuple(decoded.shape)} != {(1, self.action_horizon, 3)}"
            )
        return decoded[0]

    @torch.inference_mode()
    def inference_step(self, obs_zarr: Mapping, t: int, emb_id: int, T_max=None):
        del T_max
        if emb_id not in self.domain_by_id:
            raise KeyError(f"unknown Action Flow embodiment id {emb_id}")
        if int(t) == 0 or not hasattr(self, "_sim_action_queue"):
            self._sim_action_queue = []
        if not self._sim_action_queue:
            actions = self._predict_native_chunk(obs_zarr, emb_id)
            start, stop = _resolve_action_chunk_execution_slice(
                decoded_horizon=int(actions.shape[0]),
                start_index=self.action_chunk_start_index,
                replan_every=getattr(self, "replan_every", None),
            )
            self._sim_action_queue = [actions[index] for index in range(start, stop)]
        action = self._sim_action_queue.pop(0)
        return action.detach().float().cpu().numpy().astype(np.float32, copy=False)
...
def load_algo_from_ckpt(
    ...
    cfg, hparams, full_cfg = _checkpoint_model_config(ckpt, config_path)
    ...
    if legacy_cfg is not None:
        algo = instantiate(legacy_cfg, norm_stats=norm_stats)
    else:
        wrapper_target = str(OmegaConf.select(cfg, "model._target_", default=""))
        if not wrapper_target.endswith("ActionFlowModelWrapper"):
            raise RuntimeError(
                "model.pipeline evaluator bridge is restricted to ActionFlowModelWrapper"
            )
        if full_cfg is None:
            raise RuntimeError("Action Flow rollout requires the full resolved config")
        train_datasets = OmegaConf.select(full_cfg, "data.train_datasets")
        valid_datasets = OmegaConf.select(full_cfg, "data.valid_datasets")
        domains = [] if train_datasets is None else list(train_datasets)
        valid_domains = [] if valid_datasets is None else list(valid_datasets)
        if len(domains) != 1 or valid_domains != domains:
            raise RuntimeError(
                "Action Flow rollout requires one identical train/validation domain"
            )
        base_algo = instantiate(pipeline_cfg)
        algo = _ActionFlowRolloutAlgo(
            base_algo,
            norm_stats=norm_stats,
            domain=str(domains[0]),
            action_horizon=int(OmegaConf.select(cfg, "model.action_horizon")),
            action_dim=int(OmegaConf.select(cfg, "model.action_dim")),
        )
...
def strict_no_rollout_preflight(
    ...
    for stage in algo.policy.stages:
        ...
        action_decoder = getattr(stage, "action_decoder", None)
        decoder_dims = getattr(action_decoder, "action_dims", None)
        if decoder_dims is not None and emb_name in decoder_dims:
            action_dims.append(int(decoder_dims[emb_name]))
    explicit_token_dim = getattr(algo, "model_token_dim", None)
    if explicit_token_dim is not None:
        action_dims.append(int(explicit_token_dim))
    # A composite policy may expose the same action width through more than
    # one audited interface; uniqueness matters, not the number of sightings.
