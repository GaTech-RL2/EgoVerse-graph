...
from egomimic.models.oat.model.common.misc import get_autocast_context, str_to_dtype
from egomimic.models.oat.tokenizer.oat.model.norm import Fp32LayerNorm
...
def expand_emb_from_ps(emb, ps):
    return torch.cat(
        [
            einops.repeat(emb_i, "d -> 1 n d", n=shape.numel())
            for emb_i, shape in zip(emb, ps)
        ],
        dim=1,
    )
...
class LinearHead(nn.Module):
    ...
    def forward_adaLN(
        ...
        with get_autocast_context(x, self.dtype_override):
            x = self.norm(x)
            shift, scale = adaLN_packing_fn(self.adaLN_modulation(adaLN_emb)).chunk(
                2, dim=-1
            )
    ...
    def forward(
        ...
        if adaLN_emb is not None:
            if adaLN_packing_fn is None:
                adaLN_packing_fn = partial(
                    expand_emb_from_ps, ps=[torch.Size([x.shape[1]])]
                )
