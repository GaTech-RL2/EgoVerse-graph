...
from egomimic.models.oat.perception.base_obs_encoder import BaseObservationEncoder
    ...
    def __init__(
        ...
        for port_name, attr in shape_meta["obs"].items():
            ...
            if port_type == "rgb":
                ...
                raise ValueError(
                    f"Unsupported port type {port_type} for port {port_name}."
                )

        # instantiate encoders
        if rgb_ports:
            assert vision_encoder is not None, (
                "rgb ports found but no vision_encoder provided."
            )
            vision_encoder = hydra.utils.instantiate(
                vision_encoder, shape_meta=shape_meta
            )
        else:
            del vision_encoder
            vision_encoder = None
        if text_ports:
            assert text_encoder is not None, (
                "text ports found but no text_encoder provided."
            )
        ...
        if state_ports:
            assert state_encoder is not None, (
                "state ports found but no state_encoder provided."
            )
            state_encoder = hydra.utils.instantiate(
                state_encoder, shape_meta=shape_meta
            )
        else:
            del state_encoder
            state_encoder = None
