...
The numbers remain unchanged in the 06:49 UTC live audit. All missing entries have submitted training or evaluation jobs; see the [completion campaign](libero_completion_launch_20260925.md) for current progress and measured timing. Full completion by September 25, 09:00 Pacific is not supported by the observed throughput.
...
† Eleven original comparisons have different initial states: all six Spatial runs (200 episodes each), and five LIBERO-10 runs (600 episodes each). Full 2500 episode reevaluations use the original immutable checkpoints. Their unstarted jobs were moved to available L40S capacity under `arc-paired-s03-20260925-*`; all 11 are producing rollout records at the 06:49 UTC audit. Old scores are preserved here until replacement evaluations finish. Do not interpret these entries as fully paired estimates.
...
Only 6 of 24 Transformer ARC selections have complete scores. All 18 interrupted entries now have accepted replacement jobs under `arc-dpr-20260925-*`: 15 checkpoint resumes, two fresh shared LIBERO-10 runs, and one evaluation-only job. STK2/Object is producing fresh rollout records; its old 1559 episode partial is excluded. The other 17 are queued at the latest scheduler snapshot.

Plain DP with no ARC/OAT tokenizer is submitted for both architectures on Spatial/Object/Goal/LIBERO-10. Five runs have advancing full-training optimizer counters, two Spatial jobs are queued, and OAT-DP/Goal has a queued checkpoint recovery after a CUDA launch failure. No verified baseline score exists yet; pending cells must not be read as zero. Full training remains 5001 epochs/global 1024 and full evaluation remains 2500 episodes. LIBERO-90 and the ARC+OAT hybrid remain deferred.
