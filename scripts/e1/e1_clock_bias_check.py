...
for idx in np.linspace(0, len(ds) - 1, n).astype(int):
    c = np.asarray(
        ds[int(idx)]["actions_time"], dtype=np.float64
    )  # (200, 14) eef-frame
    tok = {
        m: tk[m].transform({"actions_cartesian": c.copy()})["actions_cartesian"]
        for m in tk
    }
    ...
    for k, (xo, _, _, _) in enumerate(ARM_LAYOUT):
        ...
        t_true.append(tt)
        t_ld.append(clk["logdur"][k][-1])
        t_du.append(clk["dur"][k][-1])
...
print(
    f"  -> correlation(logdur/arcdur, polyline/span) = "
