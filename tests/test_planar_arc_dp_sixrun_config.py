...
def test_paper_ema_is_registered_checkpointed_and_uses_exact_warmup(
    ...
    parameter = wrapper.model.nets["policy"].weight
    with torch.no_grad():
        parameter.zero_()
    optimizer = torch.optim.SGD([parameter], lr=1.0)

    # Lightning calls on_before_zero_grad before the current backward/step.
    # Accumulation-only batches must therefore leave EMA untouched.
    wrapper.on_before_zero_grad(optimizer)
    assert wrapper.ema_optimization_step.item() == 0

    parameter.grad = torch.full_like(parameter, -1.0)
    wrapper.optimizer_step(0, 0, optimizer, lambda: None)
    ...
    parameter.grad = torch.full_like(parameter, -1.0)
    wrapper.optimizer_step(0, 1, optimizer, lambda: None)
    ...
    torch.testing.assert_close(
        wrapper.ema_model.nets["policy"].weight,
        torch.full_like(parameter, expected_decay + 2.0 * (1.0 - expected_decay)),
    )
