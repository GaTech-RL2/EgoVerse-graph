"""Pipeline adapter for the repository's standard Diffusion Policy."""
...
def _scheduler_value(scheduler, name: str):
    """Read a setting from either a local or a diffusers scheduler."""
...
class MultiDomainDiffusionPolicyStage(Stage):
    """Route each action domain through its width-specific epsilon policy.

    Training predicts sampled Gaussian noise at a random diffusion timestep.
    Evaluation and rollout execute the policy's complete reverse process.
    """
