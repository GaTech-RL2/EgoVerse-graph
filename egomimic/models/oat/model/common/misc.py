...
def powers_of_two(min_val, max_val):
    return [
        2**i for i in range(int(min_val).bit_length() - 1, int(max_val).bit_length())
    ]
...
def get_bf16_context(
    enable_bf16: bool = detect_bf16_support(), device_type: str = "cuda"
):
    """
...
def get_autocast_context(
    x: Union[torch.Tensor, List[torch.Tensor]], dtype_override: torch.dtype
):
