...
class PositionalEmbedding(nn.Module):
    ...
    def __init__(
        ...
        max_sizes: List[
            int
        ],  # Maximum posemb sizes per dimension (in patches/tokens, not pixels)
    ...
    def setup_posembs(self):
        if self.posemb_type in ["learnable_sum", "learnable_prod"]:
            assert len(np.unique(self.max_sizes)) == 1, (
                "All max_sizes must be the same."
            )
            ...
            if self.n_dim == 1:
                self.posembs = build_1d_sincos_posemb(
                    *self.max_sizes, embed_dim=self.dim
                )
            elif self.n_dim == 2:
                self.posembs = build_2d_sincos_posemb(
                    *self.max_sizes, embed_dim=self.dim
                )
            else:
                raise NotImplementedError(
                    "Only 1D and 2D sincos positional embeddings are supported for now."
                )
            self.posembs = nn.Parameter(self.posembs, requires_grad=False)
        else:
            raise NotImplementedError(
                f"posemb_type '{self.posemb_type}' is not supported."
            )
    ...
    def forward(self, shape: List[int]) -> torch.Tensor:
        ...
        if self.posemb_scaling == "absolute":
            ...
            raise NotImplementedError(
                f"posemb_scaling '{self.posemb_scaling}' is not supported."
            )
