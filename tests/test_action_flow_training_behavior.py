...
def _action_flow_wrapper(*, gradient_telemetry_cadence=0, **kwargs):
    warmup_steps = kwargs.pop("reconstruction_only_warmup_steps", None)
    return ModelWrapper(
        training_behavior=ActionFlowTrainingBehavior(
            gradient_telemetry_cadence=gradient_telemetry_cadence,
            reconstruction_only_warmup_steps=warmup_steps,
        ),
...
def test_training_behavior_context_is_explicit_non_owning_and_stateless():
    wrapper = _action_flow_wrapper(pipeline=_ToyAlgo())
    behavior = wrapper.training_behavior
    assert behavior.context is wrapper
    assert wrapper.diagnostic_provider.capability == "action_flow"
    with pytest.raises(AttributeError):
        _ = behavior.model
    with pytest.raises(TypeError, match="cannot own modules"):
        behavior.accidental_model = nn.Linear(1, 1)

    copied = copy.deepcopy(wrapper)
    assert copied.training_behavior.context is copied
    assert copied.training_behavior is not behavior
...
def test_reconstruction_only_warmup_is_loaded_from_training_config_tree(monkeypatch):
    ...
    assert wrapper.training_behavior.reconstruction_only_warmup_steps == 2
    checkpoint = {}
...
def test_distributed_gradient_makes_strided_autograd_values_contiguous(monkeypatch):
    ...
    reduced = ActionFlowTrainingBehavior._distributed_gradient(gradient)
...
def test_action_flow_wrapper_uses_standard_optimizer_config(monkeypatch):
    ...
    assert type(wrapper) is ModelWrapper
...
def test_action_flow_wrapper_logs_composite_optimizer_family_rates(monkeypatch):
    ...
    wrapper.training_behavior._log_composite_optimizer_learning_rates()
...
def test_action_flow_wrapper_has_no_specialized_route_or_model_imports():
    ...
    sources = (
        root / "egomimic/pl_utils/training_behavior_action_flow.py",
        root / "egomimic/eval/action_flow_diagnostic_forward.py",
        root / "egomimic/pipeline/action_flow_topology.py",
        root / "egomimic/utils/tensor_tree.py",
    )
    for path in sources:
...
def test_action_flow_diagnostics_are_deterministic_complete_and_ad_safe():
    ...
    assert wrapper.training_behavior.encoder_e is encoder
    assert wrapper.training_behavior.field_v is field
    assert wrapper.training_behavior.decoder_g is decoder
