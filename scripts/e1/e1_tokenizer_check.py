...
hashes = [line.strip() for line in open(sys.argv[4])][:n_eps]
...
fast = TokenizeBimanualArcLengthE1(
    min_distance_unit=D,
...
for h in hashes:
    ...
    for s in rng.integers(0, len(ep) - W, size=per_ep):
        ...
        a = parent.transform({"actions_cartesian": chunk.copy()})["actions_cartesian"]
        t_par += time.perf_counter() - t0
        t0 = time.perf_counter()
        b = fast.transform({"actions_cartesian": chunk.copy()})["actions_cartesian"]
        t_fast += time.perf_counter() - t0
        d_tok.append(np.abs(a - b).max())
        da = parent.detokenize(a, action_horizon=H)
        db = fast.detokenize(b, action_horizon=H)
        d_det.append(np.abs(da - db).max())
        n += 1
print(
    f"{n} chunks: token max|diff| = {max(d_tok):.3e} (median {np.median(d_tok):.3e}); detok max|diff| = {max(d_det):.3e}"
)
print(
    f"per-sample: parent {1e3 * t_par / n:.1f} ms, fast {1e3 * t_fast / n:.2f} ms (x{t_par / max(t_fast, 1e-9):.0f})"
)
