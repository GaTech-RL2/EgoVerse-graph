...
from egomimic.models.oat.tokenizer.oat.model.head import LinearHead
from egomimic.models.oat.tokenizer.oat.model.pos_emb import PositionalEmbeddingAdder
from egomimic.models.oat.tokenizer.oat.model.sample_emb import SampleEmbedder
...
def create_causal_last_mask(
    ...
    causal_reg_mask = torch.triu(
        torch.ones((register_len, register_len), dtype=torch.bool, device=device),
        diagonal=1,
    )
...
class RegisterEncoder(nn.Module):
    def __init__(
        ...
        self.pos_emb = PositionalEmbeddingAdder(
            emb_dim,
            max_sizes=[
                sample_horizon,
            ],
    ...
    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        ...
        # transformer forward
        causal_last_attn_mask = create_causal_last_mask(
            T, self.num_registers, str(sample.device)
        )
