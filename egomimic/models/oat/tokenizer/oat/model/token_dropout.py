...
from egomimic.models.oat.model.common.misc import is_power_of_two, powers_of_two
...
class MaskedNestedDropout(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            mask = keep_ks.unsqueeze(1) <= torch.arange(N, device=x.device).unsqueeze(
                0
            )  # (B, N)
            x[mask] = self.dropout_mask_token
        else:
            if eval_keep_k is not None:
                mask = torch.tensor(eval_keep_k, device=x.device).unsqueeze(
                    1
                ) <= torch.arange(N, device=x.device).unsqueeze(0)  # (B, N)
                x[mask] = self.dropout_mask_token
        return x
