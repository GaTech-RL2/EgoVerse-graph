...
class ActionChunkCoordinateFrameTransform(Transform):
    ...
    def transform(self, batch):
        ...
        # Compute the relative homogeneous transforms. This deliberately uses
        # NumPy rather than Project Aria's Sophus bindings: these shared
        # transforms are also needed by the Yam runtime, whose i2rt dependency
        # requires an incompatible rerun-sdk version.
        target_matrix = target_world_to_matrix_fn(target_world[None, :])[0]
        chunk_mats = to_matrix_fn(chunk_world)
        if self.inverse:
            chunk_mats = np.linalg.inv(target_matrix) @ chunk_mats
        else:
            chunk_mats = target_matrix @ chunk_mats
