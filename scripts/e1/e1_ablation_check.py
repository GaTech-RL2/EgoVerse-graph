...
hashes = [line.strip() for line in open(sys.argv[4]) if line.strip()][:n_eps]
D, M, dt, W, H = 0.40, 100, 1 / 30, 200, 33
toks = {
    "profile": TokenizeBimanualArcLengthE1(
        min_distance_unit=D,
        resampled_vector_length=M,
        dt=dt,
        velocity_norm="path",
        velocity_mode="profile",
    ),
    "logdur": TokenizeBimanualArcLengthE1(
        min_distance_unit=D,
        resampled_vector_length=M,
        dt=dt,
        velocity_norm="path",
        velocity_mode="logdur",
    ),
    "logdur_smooth": TokenizeBimanualArcLengthE1(
        min_distance_unit=D,
        resampled_vector_length=M,
        dt=dt,
        velocity_norm="path",
        velocity_mode="logdur",
        progress_smooth_hz=3.0,
    ),
}
...
for h in hashes:
    ...
    for side in ("left", "right"):
        ...
        cols += [
            x[:, :3],
            R.from_quat(q[:, [1, 2, 3, 0]]).as_euler("ZYX"),
            np.zeros((len(x), 1)),
        ]
    ...
    for s in rng.integers(0, len(ep) - W, size=per_ep):
        ...
        for k, tk in toks.items():
            ...
            for j, (xo, _, _, _) in enumerate(ARM_LAYOUT):
                ...
                res[k]["anchor"].append(
                    np.linalg.norm(dec[0, xo : xo + 3] - gt[0, xo : xo + 3])
                )
                wp = a[:, xo : xo + 3]
                cum = cumulative_arc_length(wp)
                gt_pos = (
                    chunk[:, xo : xo + 3]
                    if tk.progress_smooth_hz is None
                    else lowpass_positions(
                        chunk[:, xo : xo + 3], tk.progress_smooth_hz, 30.0
                    )
                )
                ...
                if s_chk > 1e-3:
                    t_true = (
                        float(np.interp(s_chk, gt_cum, np.arange(len(gt_cum)))) * dt
                    )
                    t_pred = float(np.interp(s_chk, cum, clocks[j]))
                    res[k]["ratio"].append(t_pred / max(t_true, 1e-6))
                    u = np.linspace(0, s_chk, M)
                    res[k]["d_clock"].append(
                        np.sqrt(
                            np.mean(
                                (
                                    np.interp(u, cum, clocks[j])
                                    - np.interp(u, gt_cum, np.arange(len(gt_cum))) * dt
                                )
                                ** 2
                            )
                        )
                ...
                if k == "logdur":
                    row0.append(a[0, 14 + j])
                    rows.append(a[1:, 14 + j])
                    clipped += int(np.sum(np.abs(a[1:, 14 + j]) >= LOGDUR_CLIP - 1e-9))
        # jitter inflation: raw vs 3 Hz-smoothed arc length over the first 100 frames, per arm
        for xo, _, _, _ in ARM_LAYOUT:
            p = chunk[:100, xo : xo + 3]
            lr, ls = (
                cumulative_arc_length(p)[-1],
                cumulative_arc_length(lowpass_positions(p, 3.0, 30.0))[-1],
            )
...
for k, r in res.items():
    ...
    print(
        f"  {k:14s} codec E_time RMS {np.sqrt(np.mean(e**2))*100:.2f} cm (median {np.median(e)*100:.2f}); "
...
print(
    f"  logdur row0 (log slowness s/m): median {np.median(row0):.2f} -> {np.exp(-np.median(row0)):.3f} m/s; q10/q90 {np.quantile(row0,.1):.2f}/{np.quantile(row0,.9):.2f}"
)
print(
    f"  logdur rows1+ (log rel. duration): mean {rows.mean():.3f}, sd {rows.std():.3f}, q01/q99 {np.quantile(rows,.01):.2f}/{np.quantile(rows,.99):.2f}, clipped {clipped}/{rows.size}"
)
...
print(
    f"  jitter inflation L_raw/L_smooth(3 Hz) over 100 frames: all {np.median(infl):.3f}; slow half {np.median(slow):.3f}; fast half {np.median(fast):.3f} "
    f"(tempo median split at {np.median(tempo):.3f} m/s)"
)
ok = (
    all(
        np.median(res[k]["ratio"]) > 0.9 and np.median(res[k]["ratio"]) < 1.1
        for k in toks
    )
    and clipped < 0.01 * rows.size
)
