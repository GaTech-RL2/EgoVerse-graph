...
WT, exp, root = (
    ...
    sys.argv[2],
    sys.argv[3],
)  # exp = experiment suffix (arcvel, arclogdur_ps, ...)
with initialize_config_dir(
    config_dir=f"{WT}/egomimic/hydra_configs", version_base=None
):
    cfg = compose(
        config_name="train_zarr_cartesian",
        overrides=[
            f"+experiment=e1/{os.environ.get('E1_EXP', 'fold')}_{exp}",
            "e1.spread=smoke",
            f"e1.train_root={root}",
            f"e1.valid_root={root}",
            "e1.h_match_frames=40",
            "seed=0",
        ],
    )
...
print(
    f"[{variant}] dataset built in {time.time()-t0:.1f}s: {len(ds)} samples, leaves={len(ds.datasets)}"
)
...
if variant != "time":
    ...
    detok = TokenizeBimanualArcLengthE1(
    ...
    if cfg.e1.get("anchor_sampler"):
        ...
        smp = build_anchor_sampler(
            ds, **OmegaConf.to_container(cfg.e1.anchor_sampler, resolve=True)
        )
        draws = np.array(list(iter(smp))[:20000])
        print(
            f"  anchor sampler: {len(smp)} draws/epoch, first 20k draws hit {len(np.unique(draws))} distinct anchors"
        )
errs, clocks = [], []
for idx in np.linspace(0, len(ds) - 1, 12).astype(int):
    ...
    for k, (xo, _, _, vsl) in enumerate(ARM_LAYOUT):
        e = np.sqrt(
            np.mean(np.sum((dec[:, xo : xo + 3] - gt[:40, xo : xo + 3]) ** 2, axis=1))
        )
        errs.append(e)
        wp = a[:100, xo : xo + 3]
        ...
        if s_chk > 1e-3:
            ...
            clocks.append(
                (round(float(cum[-1]), 3), round(t_true, 2), round(t_pred, 2))
            )
if variant != "time":
    ratios = [p / t for _, t, p in clocks if t > 0]
    print(
        f"  token shape {a.shape}; codec E_time(40f) RMS over arms/samples = {np.sqrt(np.mean(np.square(errs))):.4f} m "
