...
COMMON="ICE_LAUNCH_MODE=run,ICE_REPO=$REPO,ICE_EXPECTED_HEAD=$HEAD,ICE_PYTHON=$PY,ICE_WANDB_ENTITY=rl2-group,ICE_WANDB_PROJECT=$PROJ,ICE_WANDB_GROUP=$SWEEP,ICE_EXPECTED_ACCOUNT=ece,ICE_EXPECTED_PARTITION=coe-gpu,ICE_EXPECTED_QOS=coe-ice,ICE_OUTPUT_DIR=$OUT,ICE_WANDB_RUN_ID=${WANDB_RUN_ID:-aidan-ct2-${TAG}-${STAMP}},ICE_LIMIT_TRAIN_BATCHES=1.0,ICE_LIMIT_VAL_BATCHES=8,ICE_TRAIN_BATCH_SIZE=32,ICE_VALID_BATCH_SIZE=32"
