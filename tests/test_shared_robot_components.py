...
def test_pose_and_sample_metrics_need_only_graph_prediction_contract(tmp_path):
    class Graph:
        def forward_eval(self, batch):
            return {source: {"pred_action": torch.randn_like(row["actions_cartesian"])}
    ...
    evaluator = BimanualCartesianEval(
        pose_metrics=True, rkl_samples=3, viz_every_n_epochs=0,
        group_options={"train_viz": {"rkl_samples": 1, "limit_batches": 1}},
    )
    evaluator.trainer = SimpleNamespace(
        default_root_dir=str(tmp_path), current_epoch=0, is_global_zero=True,
        lightning_module=SimpleNamespace(log_dict=lambda metrics, **kw: logged.append(metrics)),
    )
