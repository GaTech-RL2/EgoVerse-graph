"""HPT transformer CORE primitives relocated from ``models/hpt_nets.py``.

Role home for the HPT trunk building blocks (Perceiver-IO style attention): the
:class:`CrossAttention` / :class:`Attention` / :class:`MLP` layers, the
:class:`BlockWithMasking` encoder block, the masked
:class:`MultiheadAttention` wrapper, and the :class:`SimpleTransformer` stack.
Consumed by the HPT stems (cross-attn latent pooling) and heads
(decoder blocks). Moved here verbatim (class bodies byte-identical) in the
models/ hierarchy pass; only the import header + the ``_LAYER_NORM`` default
moved with :class:`SimpleTransformer`.
"""
