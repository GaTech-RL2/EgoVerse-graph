...
def test_latent_only_stopgrad_blocks_both_clean_fm_routes_not_shared_condition():
    ...
    assert all(
        gradient is None or torch.count_nonzero(gradient) == 0 for gradient in gradients
    )
...
def test_latent_only_sg_preserves_all_action_flow_gradients_and_diagnostics():
    ...
    for mode in ("full", "all_stopgrad"):
        ...
        gradients = torch.autograd.grad(
            batch["log/action_flow_action_velocity"],
            params,
            allow_unused=True,
            retain_graph=True,
        )
    ...
    for key in (
        ...
        "action_flow/velocity_residual",
        "action_flow/decoded_velocity_residual",
    ):
        torch.testing.assert_close(
            results[0][0][key], results[1][0][key], rtol=0, atol=0
        )
