...
@pytest.mark.parametrize("mode", [None, "full", "target_stopgrad", "all_stopgrad"])
def test_flow_gradient_option_is_forwarded_only_when_present(mode):
    assert trainer._flow_clean_gradient_kwargs({"clean_gradient_mode": "all_stopgrad"}) == {}
    assert trainer._flow_clean_gradient_kwargs({"flow_clean_gradient_mode": mode}) == {
        "flow_clean_gradient_mode": mode
    }
...
def test_periodic_generation_preserves_optimizer_rng_and_logs_exact_cloud_once(
    ...
    for every in (0, 1):
        config = {
            ...
            "source_key": "source_3d", "training_noise": "fresh_gaussian",
