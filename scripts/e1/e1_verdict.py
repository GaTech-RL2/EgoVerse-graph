...
def read(p):
    ...
    def g(k):
        return next(
            (v for kk, v in r.items() if kk.startswith("Valid/E1/" + k + "/")), None
        )

    xyz = g("xyz_mse_full") if any("xyz_mse_full" in k for k in r) else g("xyz_mse")
    return dict(
        am=g("arcmatch_paired_mse"),
        amg=g("arcmatch_gtspan_paired_mse"),
        span=g("arcmatch_span_m"),
        gspan=g("arcmatch_gtspan_span_m"),
        xyz=xyz,
    )
...
for (fam, fm), runs in sorted(per_fam.items()):
    ...
    def a(k):
        return [r[2][k] for r in runs if r[2][k] is not None]

    def m(k):
        return (sum(a(k)) / len(a(k))) if a(k) else None
...
for (fam, fm), runs in sorted(per_fam.items()):
    for name, tag, rec in sorted(runs):
        print(
            "  {:26} {:6} arcmatch {:>9} gtspan {:>9} xyz {:>9}".format(
