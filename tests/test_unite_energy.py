...
def test_energy_artifacts_preserve_validation_across_slurm_attempts(
    ...
    for job_id, restart in (("5691426", 0), ("5691426", 1), ("5692000", 0)):
        ...
        evaluator.trainer = SimpleNamespace(
            current_epoch=0, global_step=20001, global_rank=0
        )
        ...
        torch.testing.assert_close(
            payload["domains"]["pushshapes_sim_u_socket"]["predictions"], samples
        )
...
def test_typed_usocket_energy_artifact_binds_full_metric_identity(
    tmp_path, monkeypatch
):
...
def test_energy_artifact_records_provenance_and_per_condition_outputs(
    tmp_path, monkeypatch
):
    monkeypatch.delenv("SLURM_JOB_ID", raising=False)
    monkeypatch.delenv("SLURM_RESTART_COUNT", raising=False)
